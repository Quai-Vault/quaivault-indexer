...
BEGIN
    ...
    EXECUTE format('DROP TABLE IF EXISTS %I.wallet_module_events CASCADE', schema_name);
...
BEGIN
    ...
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.wallet_modules (
            ...
            enabled_at_block BIGINT,
            enabled_at_tx TEXT,
            disabled_at_block BIGINT,
            disabled_at_tx TEXT,
            is_active BOOLEAN DEFAULT TRUE,
            last_event_block BIGINT NOT NULL,
            last_event_block_hash TEXT,
            last_event_tx TEXT NOT NULL,
    ...
    -- Append-only module enable/disable lifecycle history
    EXECUTE format('
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_wallet_module_events_wallet_block ON %I.wallet_module_events(wallet_address, event_block DESC, log_index DESC)', schema_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_wallet_module_events_module_block ON %I.wallet_module_events(module_address, event_block DESC, log_index DESC)', schema_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_wallet_module_events_block ON %I.wallet_module_events(event_block)', schema_name);
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_module_executions_inventory ON %I.module_executions(wallet_address, module_address, executed_at_block DESC, log_index DESC NULLS LAST)', schema_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_module_executions_wallet_order ON %I.module_executions(wallet_address, executed_at_block DESC, log_index DESC NULLS LAST)', schema_name);
    ...
    -- Atomic, idempotent, and ordered wallet-module lifecycle application.
    EXECUTE format('
        CREATE OR REPLACE FUNCTION %I.apply_wallet_module_event(
            p_wallet TEXT, p_module TEXT, p_event_type TEXT, p_event_block BIGINT,
            p_event_block_hash TEXT, p_event_tx TEXT, p_log_index INTEGER
        )
        RETURNS TEXT
        LANGUAGE plpgsql
        SECURITY INVOKER
        ...
        BEGIN
            ...
            SELECT EXISTS (
                ...
                WHERE wallet_address = normalized_wallet AND module_address = normalized_module
            ...
            IF p_event_type = ''enabled'' THEN
                INSERT INTO wallet_modules (
                    ...
                    NULL, NULL, TRUE, p_event_block, lower(p_event_block_hash),
                    normalized_tx, p_log_index, NOW()
                )
            ...
            ELSE
                INSERT INTO wallet_modules (
                    ...
                    p_event_block, normalized_tx, FALSE, p_event_block,
                    lower(p_event_block_hash), normalized_tx, p_log_index, NOW()
                )
                ON CONFLICT (wallet_address, module_address) DO UPDATE SET
            ...
            IF changed_rows = 0 THEN
                ...
                RETURN ''out_of_order'';
            END IF;
            IF p_event_type = ''disabled'' AND NOT projection_existed THEN RETURN ''orphan_applied''; END IF;
            RETURN ''applied'';
        END;
        $func$
    ', schema_name, schema_name);

    -- Targeted module rollback reconstructs projections from remaining history.
    EXECUTE format('
        ...
        BEGIN
            ...
            DELETE FROM wallet_modules wm USING qv_affected_modules affected
            ...
            WITH latest AS (
                SELECT DISTINCT ON (events.wallet_address, events.module_address) events.*
                FROM wallet_module_events events
                JOIN qv_affected_modules affected USING (wallet_address, module_address)
                ...
                SELECT DISTINCT ON (events.wallet_address, events.module_address)
                    events.wallet_address, events.module_address, events.event_block, events.event_tx
                FROM wallet_module_events events
                JOIN qv_affected_modules affected USING (wallet_address, module_address)
            ...
            SELECT latest.wallet_address, latest.module_address,
                   latest_enable.event_block, latest_enable.event_tx,
                   ...
                   latest.event_type = ''enabled'', latest.event_block,
                   latest.event_block_hash, latest.event_tx, latest.log_index, NOW()
            FROM latest
            LEFT JOIN latest_enable USING (wallet_address, module_address);
    ...
    -- Stable public inventory contract with freshness and execution aggregates.
    EXECUTE format('
        ...
        AS $func$
            ...
            LEFT JOIN LATERAL (
                ...
                ) ORDER BY modules.is_active DESC, modules.module_address) AS modules
    ...
    -- Correctness-first recovery for general reorgs affecting mutable projections.
    EXECUTE format('
    ...
    EXECUTE format('ALTER TABLE %I.wallet_module_events ENABLE ROW LEVEL SECURITY', schema_name);
    ...
    EXECUTE format('DROP POLICY IF EXISTS "Public read access" ON %I.wallet_module_events', schema_name);
    EXECUTE format('CREATE POLICY "Public read access" ON %I.wallet_module_events FOR SELECT USING (true)', schema_name);
    ...
    EXECUTE format('DROP POLICY IF EXISTS "Service write access" ON %I.wallet_module_events', schema_name);
    EXECUTE format('CREATE POLICY "Service write access" ON %I.wallet_module_events FOR ALL USING (auth.role() = ''service_role'') WITH CHECK (auth.role() = ''service_role'')', schema_name);
    ...
    -- Mutating maintenance/lifecycle functions are service-only. PostgreSQL
    -- grants function execution to PUBLIC by default, so revoke explicitly after
    -- the blanket grants above.
    EXECUTE format('REVOKE ALL ON FUNCTION %I.apply_wallet_module_event(TEXT, TEXT, TEXT, BIGINT, TEXT, TEXT, INTEGER) FROM PUBLIC, authenticated, anon', schema_name);
    EXECUTE format('REVOKE ALL ON FUNCTION %I.rollback_wallet_module_events_after(BIGINT) FROM PUBLIC, authenticated, anon', schema_name);
    EXECUTE format('REVOKE ALL ON FUNCTION %I.reset_indexed_data(BIGINT, BIGINT, TEXT) FROM PUBLIC, authenticated, anon', schema_name);
    EXECUTE format('GRANT EXECUTE ON FUNCTION %I.apply_wallet_module_event(TEXT, TEXT, TEXT, BIGINT, TEXT, TEXT, INTEGER) TO service_role', schema_name);
    EXECUTE format('GRANT EXECUTE ON FUNCTION %I.rollback_wallet_module_events_after(BIGINT) TO service_role', schema_name);
    EXECUTE format('GRANT EXECUTE ON FUNCTION %I.reset_indexed_data(BIGINT, BIGINT, TEXT) TO service_role', schema_name);
    ...
    BEGIN
        EXECUTE format('ALTER PUBLICATION supabase_realtime ADD TABLE %I.wallet_module_events', schema_name);
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;
